...
"""
function softmin_lasso(A, y, lambda; tau, step=1.0, x0=nothing, kwargs...)
...
mutable struct SoftminLassoWorkspace{AType,YType,V,T}
    A::AType
    y::YType
    x::V
    x_previous::V
...
function SoftminLassoWorkspace(A, y; step=1.0, x0=nothing)
...
function _softmin_lasso_single_tau!(
    ...
    data_loss, fresh_gradient = softmin_lasso_loss_gradient(A, y, x, tau)
    copyto!(gradient, fresh_gradient)
    ...
    for iteration in 1:maxiter
        ...
        while true
            ...
            trial_data_loss = softmin_lasso_loss(A, y, x, tau)
...
function _softmin_return_info(workspace, lambda, tau, last_info, stage_infos)
    weights = softmin_lasso_weights(workspace.A, workspace.y, workspace.x, tau)
    return (
        x=workspace.x,
